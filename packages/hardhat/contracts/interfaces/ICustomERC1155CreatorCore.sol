...
import "../CustomCreatorCore.sol";

/**
 * @dev Core ERC1155 creator interface
 */
interface ICustomERC1155CreatorCore is ICustomCreatorCore {
     ...
     * @param tokenIds  - Can be a single element array (all recipients get the same amount) or a multi-element array
     ...
     */
    function mintBaseNew(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts, string[] calldata uris) external returns (uint256[] memory);
     ...
     * @return Returns list of tokenIds minted
     */
    function mintExtensionNew(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts, string[] calldata uris) external returns (uint256[] memory);
