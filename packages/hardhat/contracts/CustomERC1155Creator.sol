...
/// @author: manifold.xyz
/// @dev: customised by MetaFactory

import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";
import "@manifoldxyz/libraries-solidity/contracts/access/AdminControl.sol";
import "./CustomERC1155CreatorCore.sol";

/**
 * @dev CustomERC1155Creator implementation
 */
contract CustomERC1155Creator is AdminControl, ERC1155, CustomERC1155CreatorCore {

    mapping(address => mapping (uint256 => bool)) public wasEverMinted;
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155, CustomERC1155CreatorCore, AdminControl) returns (bool) {
        return CustomERC1155CreatorCore.supportsInterface(interfaceId) || ERC1155.supportsInterface(interfaceId) || AdminControl.supportsInterface(interfaceId);
    }
     ...
     * @dev wasEverMinted requirement checks whether the token id exists.
     */
    function mintBaseNew(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts, string[] calldata uris) public virtual override nonReentrant adminRequired returns(uint256[] memory) {
        for (uint i = 0; i < tokenIds.length; i++) {
            require(!wasEverMinted[address(this)][tokenIds[i]], "Invalid ids, tokenId already exists");
            wasEverMinted[address(this)][tokenIds[i]] = true;
        }
        return _mintNew(address(this), to, tokenIds, amounts, uris);
    }
    ...
    function mintBaseExisting(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts) public virtual override nonReentrant adminRequired {
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(wasEverMinted[address(this)][tokenIds[i]], "Invalid ids, tokenId does not exist");
        }
     ...
     */
    function mintExtensionNew(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts, string[] calldata uris) public virtual override nonReentrant extensionRequired returns(uint256[] memory) {
        for (uint i = 0; i < tokenIds.length; i++) {
            require(!wasEverMinted[address(this)][tokenIds[i]], "Invalid ids, tokenId already exists for extension");
            wasEverMinted[msg.sender][tokenIds[i]] = true;
        }
        return _mintNew(msg.sender, to, tokenIds, amounts, uris);
    }
    ...
    function mintExtensionExisting(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts) public virtual override nonReentrant extensionRequired {
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(wasEverMinted[address(msg.sender)][tokenIds[i]], "Invalid ids, tokenId does not exist for extension");
        }
     ...
     * @dev Mint new tokens
     */
    function _mintNew(address extension, address[] memory to, uint256[] calldata ids, uint256[] memory amounts, string[] memory uris) internal returns(uint256[] memory tokenIds) {
        if (to.length > 1) {
            // Multiple receiver.  Give every receiver the same new token
            tokenIds = new uint256[](1);
            require(uris.length <= 1 && (amounts.length == 1 && ids.length == 1 || to.length == amounts.length && to.length == ids.length), "Invalid input");
        } else {
            // Single receiver.  Generating multiple tokens
            tokenIds = new uint256[](ids.length);
            require(uris.length == 0 || amounts.length == uris.length && ids.length == uris.length, "Invalid input");
        }
        // Assign tokenIds
        for (uint i = 0; i < tokenIds.length; i++) {
            _tokenCount++;
